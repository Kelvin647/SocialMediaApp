    ...
    function displayPost(post) {
        const postElement = `
            <div class="post" id="post-${post.id}">
                <p>${post.content}</p>
                ${post.media ? (post.media.includes('image') ? `<img src="${post.media}" alt="Post Media">` : `<video controls><source src="${post.media}" type="video/mp4">Your browser does not support the video tag.</video>`) : ''}
                <div>
